...
async def setup_api_keys(request: APIKeysRequest):
    ...
    # Add Play.ht API key and User ID support
    playht_key = (request.playht_key or "").strip() if hasattr(request, 'playht_key') else os.getenv("PLAYHT_API_KEY")
    playht_user_id = (request.playht_user_id or "").strip() if hasattr(request, 'playht_user_id') else os.getenv("PLAYHT_USER_ID")
    
    if playht_key and playht_user_id:
        try:
            # Test Play.ht connection
            headers = {
                "Authorization": f"Bearer {playht_key}",
                "X-User-ID": playht_user_id
            }
            response = requests.get("https://api.play.ht/api/v2/voices", headers=headers)
            if response.status_code == 200:
                api_keys_storage["playht_key"] = playht_key
                api_keys_storage["playht_user_id"] = playht_user_id
            else:
                raise Exception(f"Invalid Play.ht credentials: {response.status_code}")
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid Play.ht credentials: {str(e)}")
...
async def get_api_keys_status():
    ...
    return {
        ...
        "sarvam_configured": "sarvam_key" in api_keys_storage or bool(os.getenv("SARVAM_API_KEY")),
        "openai_configured": "openai_key" in api_keys_storage,
        "playht_configured": ("playht_key" in api_keys_storage and "playht_user_id" in api_keys_storage) or 
                            (bool(os.getenv("PLAYHT_API_KEY")) and bool(os.getenv("PLAYHT_USER_ID")))
    }
...
def get_api_keys():
    ...
    # Add Play.ht fallback to .env
    if "playht_key" not in api_keys_storage or not api_keys_storage["playht_key"]:
