...
from app.routes.api_keys import get_api_keys
from app.services.tts_service import ensure_audio_is_generated, ensure_hindi_audio_is_generated, ensure_language_audio_is_generated
from app.services.voice_clone_service import ensure_voice_cloned_audio_is_generated
from app.services.video_service import create_video_with_audio
from app.services.hindi_service import generate_hindi_script_with_google
from app.services.language_service import translate_to_language
...
# In-memory storage for media and voice samples
media_storage = {}
voice_samples_storage = {}

@router.post("/{paper_id}/upload-voice-sample")
async def upload_voice_sample(
    ...
    try:
        ...
        voice_dir = f"temp/voice_samples/{paper_id}"
        Path(voice_dir).mkdir(parents=True, exist_ok=True)
        
        # Save the voice sample (Coqui accepts various formats: wav, mp3, etc.)
        file_ext = os.path.splitext(voice_sample.filename)[1] or '.wav'
        voice_sample_path = os.path.join(voice_dir, f"voice_sample{file_ext}")
        with open(voice_sample_path, "wb") as f:
            content = await voice_sample.read()
            f.write(content)
        
        # Store in memory
        voice_samples_storage[paper_id] = voice_sample_path
        
        return {
            "message": "Voice sample uploaded successfully",
            "paper_id": paper_id,
            "voice_sample_path": os.path.basename(voice_sample_path)
        }
        
    except Exception as e:
        print(f"Error uploading voice sample: {str(e)}")
        print(traceback.format_exc())
...
async def generate_audio(
    ...
    print(f"using voice selection:, {request.voice_selection}")
    print(f"Generating audio for paper ID: {paper_id}")
    print(f"Use voice cloning: {request.use_voice_cloning}")
    ...
    if request.use_voice_cloning:
        # Coqui TTS is free and local - no API key needed!
        
        if paper_id not in voice_samples_storage:
            raise HTTPException(status_code=400, detail="Voice sample not uploaded. Please upload a voice sample first.")
        ...
        try:
            ...
            print(f"Error generating voice cloned audio: {str(e)}")
            print(traceback.format_exc())
            raise HTTPException(status_code=500, detail=f"Error generating voice cloned audio: {str(e)}")
    
    # Original TTS logic (unchanged)
    if not api_keys.get("sarvam_key"):
        raise HTTPException(status_code=400, detail="Sarvam API key required for TTS")
